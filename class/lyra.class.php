...
#####################################################################################################
#
#					Module pour la plateforme de paiement Lyra
#
#####################################################################################################
...
if(!@class_exists('Lyra', false)) {
	...
	class Lyra {
		...
		// **************************************
		/**
		 * The fields to send to the Lyra platform
		 ...
		 * @var LyraResponse
		 * @access private
		 * @deprecated see LyraResponse constructor
		 */
		var $response;
		/**
		...
		*/
		...
		function _addRequestField($name, $label, $regex, $required = false,
			...
			$this->requestParameters[$name] = new LyraField($name, $label, $regex,
					$required, $length);
		...
		/**
		 * Return the list of currencies recognized by the Lyra platform
		 * @static
		...
		function getSupportedCurrencies() {
			...
			$LyraCurrencies = array();
			
			foreach($currencies as $currency) {
				$LyraCurrencies[] = new LyraCurrency($currency[0], $currency[1], $currency[2]);
			}
			
			return $LyraCurrencies;
		}
		...
		function getCurrencyNumCode($alpha3) {
			...
			return is_a($currency, 'LyraCurrency') ? $currency->num : null;
		}
		...
		*/
		 ...
		 * @param array[string]LyraField $fields
		 * @return string
		 * @access private
		 */
		...
		/**
		 * Public static method to compute a Lyra signature. Parameters must be in utf-8.
		 ...
		 * @return array[string]LyraField a list of LyraField or false if a parameter was invalid
		 * @see LyraField
		 */
		...
		function loadResponse($parameters = null, $ctx_mode = null, $key_test = null,
			...
			$this->response = new LyraResponse();
			$this->response->load($parameters, $this->getCertificate());
		}
	
		/**
		 * Return a LyraResponse object representing the result of the payment.
		 * You can provide arguments to load data as in loadResponse.
		 * @see LyraApi::loadResponse
		 * @return LyraResponse
		 * @deprecated see LyraResponse constructor
		 */
...
if(!@class_exists('LyraResponse', false)) {
	...
	class LyraResponse {
		...
		/**
		 * Internal reference to LyraApi for using util methods
		 * @var LyraApi
		 * @access private
		 */
		var $api;
		...
		/**
		 * Constructor for LyraResponse class. Prepare to analyse check url or return url call.
		 * @param array[string]string $parameters $_REQUEST by default
		 * @param string $ctx_mode
		 * @param string $key_test
		 * @param string $key_prod
		 ...
		 */
		function LyraResponse($parameters = null, $ctx_mode = null, $key_test = null, $key_prod = null, $encoding = 'UTF-8') {
				
			$this->api = new Lyra($encoding);
...
if(!@class_exists('LyraField', false)) {
	...
	class LyraField {
		 ...
		 * @return LyraField
		 */
		function LyraField($name, $label, $regex, $required = false, $length = 255) {
			$this->name = $name;
			$this->label = $label;
			$this->regex = $regex;
...
if(!@class_exists('LyraCurrency', false)) {
	...
	class LyraCurrency {
		...
		function LyraCurrency($alpha3, $num, $decimals = 2) {
			$this->alpha3 = $alpha3;
			$this->num = $num;
